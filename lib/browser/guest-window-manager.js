...
const createGuest = async function (embedder, url, referrer, frameName, options, postData) {
  ...
  if (!options.webContents) {
    ...
    await guest.loadURL(url, loadOptions);
  }
...
ipcMainInternal.on('ELECTRON_GUEST_WINDOW_MANAGER_WINDOW_OPEN', async (event, url, frameName, features) => {
  ...
  await internalWindowOpen(event, url, referrer, frameName, disposition, options, additionalFeatures, null);
});

// Routed window.open messages with fully parsed options
async function internalWindowOpen (event, url, referrer, frameName, disposition, options, additionalFeatures, postData) {
  ...
  if ((event.sender.getType() === 'webview' && event.sender.getLastWebPreferences().disablePopups) || event.defaultPrevented) {
    ...
    event.returnValue = await createGuest(event.sender, url, referrer, frameName, options, postData);
  }
}
